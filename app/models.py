...
class Grow(db.Model):
    ...
    notes = db.relationship('GrowNote', backref='author', lazy='dynamic')
    ...
    def most_recent_reading(self):
        return self.readings.order_by(Reading.timestamp.desc()).first()

    # def most_recent_image(self):
    #     most_recent_reading = self.readings.order_by(Reading.timestamp.desc()).first()
    #     if most_recent_reading:
    #         # lastpic = '/static/img/growpics/{0}/{1}/{2}.jpg'.format(self.user_id, self.id, most_recent_reading.id)
    #         lastpic = most_recent_reading.photo_path
    #         return lastpic
